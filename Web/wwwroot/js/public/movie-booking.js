...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (continueBtn) {
        continueBtn.addEventListener('click', function(e) {
            ...
            if (this.hasAttribute('disabled') || selectedSeats.length === 0) {
                e.preventDefault();
                console.log('Click prevented - no seats selected');
                return false;
            }
            
            // Here you would typically navigate to the next page or trigger the next step
            // For now, we'll just log the action
            console.log('Proceeding to next step with selected seats');
        });
    }
});
